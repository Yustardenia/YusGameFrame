...
{
    private const string SceneMenuPath = YusGameFrameEditorMenu.Root + "Utilities/Scenes/Load Scenes...";

    // 动态生成菜单项
    [MenuItem(SceneMenuPath, false, 0)]
    public static void ShowSceneMenu() { }

    // 验证函数：在这里动态添加子菜单
    [MenuItem(SceneMenuPath, true)]
    public static bool ShowSceneMenuValidate()
    {
        Menu.SetChecked(SceneMenuPath, false);
        return true;
    }

    // 注意：Unity原生不支持动态生成顶级菜单，这里用一个简单变通
    // 推荐把当前 Build Settings 里的场景列出来
    [MenuItem(YusGameFrameEditorMenu.Root + "Utilities/Scenes/Open Build Settings Scenes", false, 1)]
...
{
    [MenuItem(YusGameFrameEditorMenu.Root + "Utilities/统计代码行数 (C#)", false, 50)]
...
public class ToDoListWindow : EditorWindow
{
    ...
    [MenuItem(YusGameFrameEditorMenu.Root + "Utilities/待办事项便签 (To-Do)", false, 51)]
...
public class FavoritesWindow : EditorWindow
{
    ...
    [MenuItem(YusGameFrameEditorMenu.Root + "Utilities/资源收藏夹 (Favorites)", false, 52)]
    public static void ShowWindow()
    {
