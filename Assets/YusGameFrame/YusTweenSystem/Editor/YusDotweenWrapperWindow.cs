...
public class YusDotweenWrapperWindow : EditorWindow
{
    ...
    [MenuItem("Tools/Yus Data/L. Dotween封装/打开启用窗口")]
    ...
    private void OnGUI()
    {
        ...
        if (!dotweenInstalled)
        {
            EditorGUILayout.HelpBox("当前工程里没有检测到 DOTween。启用宏会导致编译报错。\n请先导入/安装 DOTween，再回来点“启用系统”。", MessageType.Warning);
        }

#if YUS_DOTWEEN
        EditorGUILayout.Space(12);
        GUILayout.Label("场景快速安装", EditorStyles.boldLabel);
        EditorGUILayout.HelpBox("把 YusTweenManager 挂到场景里的 YusSingletonManager 下面（没有就创建一个）。", MessageType.Info);
